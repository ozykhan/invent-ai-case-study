CREATE TYPE "public"."ingestion_chunk_status" AS ENUM('pending', 'processing', 'completed', 'failed');--> statement-breakpoint
CREATE TYPE "public"."discount_type" AS ENUM('percentage', 'fixed');--> statement-breakpoint
CREATE TYPE "public"."ingestion_job_status" AS ENUM('pending', 'splitting', 'processing', 'completed', 'failed');--> statement-breakpoint
CREATE TYPE "public"."promotion_scope" AS ENUM('product', 'category');--> statement-breakpoint
...
ALTER TABLE "ingestion_rejections" ADD CONSTRAINT "ingestion_rejections_job_id_ingestion_jobs_id_fk" FOREIGN KEY ("job_id") REFERENCES "public"."ingestion_jobs"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "ingestion_chunks_job_index_uq" ON "ingestion_chunks" USING btree ("job_id","chunk_index");--> statement-breakpoint
CREATE INDEX "ingestion_rejections_job_idx" ON "ingestion_rejections" USING btree ("job_id");--> statement-breakpoint
CREATE INDEX "products_category_base_price_idx" ON "products" USING btree ("category_id","base_price");--> statement-breakpoint
CREATE INDEX "promotions_category_window_idx" ON "promotions" USING btree ("category_id","starts_at","ends_at") WHERE "promotions"."cancelled_at" is null;--> statement-breakpoint
