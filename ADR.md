...
- **Stock is kept out of catalog entries.** It lives in `stock:{id}` counters (24 h TTL) that are rebuilt from Postgres on a miss and set after every stock write and ingestion batch. Stock changes therefore never invalidate cached prices, and cached prices never serve stale stock.
...
1. **Validate** (Zod): SKU, name and category present and length-bounded, `vendor_price` a positive decimal with at most two places, `stock` a non-negative integer. Failures become rejections with a reason.
