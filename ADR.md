...
- **Coalescing fails closed.** On a miss, one reader takes `lock:{key}` (`SET NX PX 30000` with a random token) and rebuilds. The others poll, backing off from 20 ms to 100 ms; each poll reads the entry and the lock together (one `MGET`). A waiter stops when:
  ...
  - its request's client disconnects: it stops without building or taking a released lock over.

  The holder releases the lock with an atomic compare-and-delete (a Lua script), so a holder whose lock expired cannot delete a newer holder's lock. The 30 s TTL only matters if a holder dies, and it must stay above the worst-case build, or the lock would expire under a live holder and a waiter would take over and build again. The API bounds every build: a pool acquire fails after 2 s and a statement after 5 s (`DB_POOL_ACQUIRE_TIMEOUT_MS`, `DB_STATEMENT_TIMEOUT_MS`), so the longest build, a product detail's three sequential queries, is at most 3 × (2 s + 5 s) = 21 s. The API logs a warning at startup when its timeouts break that bound.

  **Cost: an orphaned lock blocks its key for up to 30 s.** If a lock outlives its holder without being released, every reader of that key waits 5 s and gets a 503 until the TTL runs out. That happens when the holder process dies mid-build, when the release (or the `uncached` marker write) fails on a Redis error, or when the `SET NX` itself succeeds on the server but its reply times out on the client (the reader then builds directly, and its token is never released). Under the old 2 s lock the same cases cost a 2 s window of direct builds. **Rigorous alternative, not taken: lock renewal.** A short TTL (say 3 s) that the holder extends with a compare-and-`PEXPIRE` on its token every second while it builds. An orphaned lock then clears within seconds, and the TTL no longer has to cover the worst-case build. It costs a timer and a Redis round trip per second per build, and a renewal that fails mid-build has to be handled.
...
- **Degradation.** Every Redis failure on the read path is logged and falls through to Postgres. An entry built while a version read failed is served but not cached. Stock falls back to the Postgres column. A Redis that is down or erroring never fails a read. A Redis that fails at the wrong moment can: an orphaned lock (above) makes its key answer 503 after 5 s for up to 30 s.
...
- **Event-loop lag can turn waiters into bypass builds.** A Redis error on the hit, lock or poll path still degrades to a direct build (§4). On a replica pinned at its CPU limit, event-loop lag can push a command past the client's 300 ms `commandTimeout` even when Redis is healthy, and those readers then build directly. Not measured in the cold-start runs; the next run should count the `cache degraded; serving from postgres` log lines per replica.
- **Two driver-level settings can be overridden or refused.** JIT and `statement_timeout` are sent as connection startup parameters (`options=-c jit=off` and `statement_timeout`). An `options` parameter in `DATABASE_URL` (`?options=...`) replaces the JIT flag. PgBouncer rejects unknown startup parameters unless they are listed in `ignore_startup_parameters`, and then it drops them. Behind a pooler, set them on the role instead: `ALTER ROLE modaco SET jit = off; ALTER ROLE modaco SET statement_timeout = '5s';`.
...
- **An open-model load on a cold cache collapsed; it now recovers.** Two attempts at 4,900 req/s against 4 replicas, starting from an empty cache (one of them with a 30 s ramp), both collapsed:
  ...
  - **After, cold cache.** The same commands, except for the warm-up. The 4-replica "before" run used `--warmup 15s`, so its 60 s recorded window began 15 s into the collapse. The "after" runs used `--warmup 0s` and record from the first request, cold fill included, so their numbers include the fill that "before" left out. The 1-replica "before" row (the investigation's run) used `--warmup 0s` too. The "after" rows were measured at `add1e75`, before the review fixes (waiters that stop when their client disconnects among them).
    ...
    | 4 replicas, 4,900 req/s, before (15 s warm-up) | 7 of 294,001 | 0 | 46,541 | 247,453 | – | – | – | not within 5 min |
    | 4 replicas, 4,900 req/s | 256,131 of 294,001 (87%) | 14,652 | 46 | 23,172 | 1.05 ms | 5,378 ms | 138 | immediately |

  - With 1 replica the whole tail is the first 2 s of cache fill; from the 10 s mark p99 was under 8 ms. With 4 replicas 12.9% of the offered requests went unserved: 503s for 5.4% of the requests sent, plus 7.9% of the offered requests dropped by the client. Both fell in the first 15 to 20 s, while the replicas were pinned at their 1-CPU limit with 10,000 requests in flight; from then on the run held 4,900 req/s with p99 under 21 ms after the 25 s mark. The 503s were 8,923 cache waits that reached 5 s and 5,731 pool acquire timeouts.
  - A first version of the fix still let every waiter build directly when a holder failed. At 4 replicas that failed 71% of the requests sent (81% of the offered ones went unserved, counting client drops): each holder that timed out on the pool released a herd whose uncached queries starved the next holder. The takeover rule closed it.
